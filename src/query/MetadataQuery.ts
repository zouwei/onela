...
/**
 * 校验标识符（表名/列名），防止 SQL 注入
 * 只允许字母、数字、下划线和点号
 */
function sanitizeIdentifier(name: string): string {
  if (!/^[a-zA-Z_][a-zA-Z0-9_.]*$/.test(name)) {
    throw new Error(`Invalid identifier: "${name}". Only letters, numbers, underscores and dots are allowed.`);
  }
  return name;
}
...
export class MetadataQuery {
  ...
  getTableRowCountSQL(tableName: string): string {
    ...
    switch (this.dbType) {
      case 'mysql': case 'mariadb': case 'tidb':
        return `SELECT TABLE_ROWS AS count FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '${safe}'`;

      case 'postgresql': case 'postgres': case 'pg':
        return `SELECT reltuples::bigint AS count FROM pg_class WHERE relname = '${safe}'`;

      case 'sqlite': case 'sqlite3':
        return `SELECT COUNT(*) AS count FROM "${safe}"`;

      case 'sqlserver': case 'mssql':
        return `SELECT SUM(row_count) AS count FROM sys.dm_db_partition_stats WHERE object_id = OBJECT_ID('${safe}') AND index_id < 2`;

      case 'oracle': case 'oracledb':
        return `SELECT NUM_ROWS AS count FROM USER_TABLES WHERE TABLE_NAME = '${safe.toUpperCase()}'`;
  ...
  getTableSizeSQL(tableName: string): string {
    ...
    switch (this.dbType) {
      case 'mysql': case 'mariadb': case 'tidb':
        return `SELECT (DATA_LENGTH + INDEX_LENGTH) AS size_bytes FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '${safe}'`;

      case 'postgresql': case 'postgres': case 'pg':
        return `SELECT pg_total_relation_size('${safe}') AS size_bytes`;

      case 'sqlite': case 'sqlite3':
        return `SELECT page_count * page_size AS size_bytes FROM pragma_page_count(), pragma_page_size()`;

      case 'sqlserver': case 'mssql':
        return `SELECT SUM(a.total_pages) * 8 * 1024 AS size_bytes FROM sys.tables t JOIN sys.indexes i ON t.object_id = i.object_id JOIN sys.partitions p ON i.object_id = p.object_id AND i.index_id = p.index_id JOIN sys.allocation_units a ON p.partition_id = a.container_id WHERE t.name = '${safe}'`;

      case 'oracle': case 'oracledb':
        return `SELECT BYTES AS size_bytes FROM USER_SEGMENTS WHERE SEGMENT_NAME = '${safe.toUpperCase()}'`;
  ...
  getIndexesSQL(tableName: string): string {
    ...
    switch (this.dbType) {
      case 'mysql': case 'mariadb': case 'tidb':
        return `SELECT INDEX_NAME AS name, COLUMN_NAME AS column_name, NON_UNIQUE, SEQ_IN_INDEX FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '${safe}' ORDER BY INDEX_NAME, SEQ_IN_INDEX`;

      case 'postgresql': case 'postgres': case 'pg':
        return `SELECT i.relname AS name, a.attname AS column_name, NOT ix.indisunique AS non_unique FROM pg_class t JOIN pg_index ix ON t.oid = ix.indrelid JOIN pg_class i ON i.oid = ix.indexrelid JOIN pg_attribute a ON a.attrelid = t.oid AND a.attnum = ANY(ix.indkey) WHERE t.relname = '${safe}' ORDER BY i.relname`;

      case 'sqlite': case 'sqlite3':
        return `PRAGMA index_list("${safe}")`;

      case 'sqlserver': case 'mssql':
        return `SELECT i.name AS name, c.name AS column_name, CASE WHEN i.is_unique = 1 THEN 0 ELSE 1 END AS non_unique FROM sys.indexes i JOIN sys.index_columns ic ON i.object_id = ic.object_id AND i.index_id = ic.index_id JOIN sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id WHERE i.object_id = OBJECT_ID('${safe}') ORDER BY i.name, ic.key_ordinal`;

      case 'oracle': case 'oracledb':
        return `SELECT i.INDEX_NAME AS name, ic.COLUMN_NAME AS column_name, CASE WHEN i.UNIQUENESS = 'UNIQUE' THEN 0 ELSE 1 END AS non_unique FROM USER_INDEXES i JOIN USER_IND_COLUMNS ic ON i.INDEX_NAME = ic.INDEX_NAME WHERE i.TABLE_NAME = '${safe.toUpperCase()}' ORDER BY i.INDEX_NAME, ic.COLUMN_POSITION`;
  ...
  getForeignKeysSQL(tableName: string): string {
    ...
    switch (this.dbType) {
      case 'mysql': case 'mariadb': case 'tidb':
        return `SELECT CONSTRAINT_NAME AS name, COLUMN_NAME AS column_name, REFERENCED_TABLE_NAME AS ref_table, REFERENCED_COLUMN_NAME AS ref_column FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '${safe}' AND REFERENCED_TABLE_NAME IS NOT NULL`;
