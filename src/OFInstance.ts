...
class Service {
  ...
  constructor(oodbc: OODBC, _database?: string, _instance?: string) {
  ...
  // === 执行 SQL ===
  private execute(instance: any, sql: string, parameters: any[] = []): Promise<any> {
    return new Promise((resolve, reject) => {
      console.time('【onela】SQL execution time');
      if (instance) {
        instance.query(sql, parameters, (err: any, doc: any) => {
  ...
  // === 瀑布流查询 ===
  queryWaterfallList(paras: QueryParams): Promise<{ data: any[] }> {
  ...
  // === 分页查询（含总数）===
  queryEntityList(paras: QueryParams): Promise<{ data: any[]; recordsTotal: number }> {
    const { database, db } = this;
  ...
  // === 自由查询 ===
  queryEntity(paras: QueryParams): Promise<any[]> {
    const { database, db } = this;
  ...
  // === 统计聚合 ===
  statsByAggregate(paras: QueryParams): Promise<any> {
    const { database, db } = this;
   ...
   */
  insertionBatch(paras: InsertParams): Promise<any> {
      //参数
  ...
  // === 高级更新（支持 CASE WHEN）===
  updateBySenior(paras: UpdateParams): Promise<any> {
    const { database, db } = this;
  ...
  // === 物理删除 ===
  deleteEntity(paras: DeleteParams): Promise<any> {
    const { database, db } = this;
  ...
  // === 存储过程调用 ===
  call(paras: CallParas): Promise<any> {
    const { database, db } = this;
    ...
    const factory: Record<string, () => Promise<any>> = {
      MYSQL: () => {
        return new Promise((resolve, reject) => {
          const symbol = '?';
          const _self = { call: '', parameter: [] as string[], values: [] as any[] };
          const error: ProcParam[] = [];
...
// === 工厂函数 ===
function service(oodbc: OODBC, _database?: string, _instance?: string): Service {
