...
class SQLServerActionManager extends BaseActionManager {
    ...
    static createTransaction() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            conn.on('connect', function (err) {
                if (err) {
                    console.error("Error creating SQL Server transaction access connection", err); // 创建SQL Server事务访问连接出错
                } else {
    ...
    static execute(sql, parameters) {
        ...
        return new Promise((resolve, reject) => {
            ...
            conn.on('connect', function (err) {
                if (err) {
                    console.error("SQL Server access connection error", err); // SQL Server访问连接出错
                } else {
    ...
    static executeTransaction(sql, parameters, transaction) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let request = new self.Request(sql, function (err, rowCount) {
                //判断有没有出错
                if (err) {
                    console.error("Error creating SQL execution request", err); // 创建SQL执行请求出错
                    return reject(err);
                }
                else rows['rowCount'] = rowCount; //rowCount是语句执行影响行数
                // console.log("rowCount", rowCount);
