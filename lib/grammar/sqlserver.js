...
m.getParameters = function (paras) {
    ...
    for (let i in paras.keyword) {
        ...
        if (paras.keyword[i] != '') {
             ...
             * 匹配参数类型
             */
            for (let s = 0; s < paras.configs.length; s++) {
                if (paras.keyword[i].name === paras.configs[s].name) {
                    paras.keyword[i].sqlType = paras.configs[s].type || "VarChar";
                    break;
                }
            }
            /**
            ...
            if (paras.keyword[i].hasOwnProperty('operator')) {
                ...
                switch (oper) {
                    ...
                    case ">=":
                    case "<=":
                        _self.where += (oper + "@" + paras.keyword[i].name + index);
                        //参数化
                        _self.parameters.push({
                            "name": paras.keyword[i].name + index,
                            "sqlType": paras.keyword[i].sqlType || "VarChar",
                    ...
                    case "not in":
                        ...
                        for (let c in paras.keyword[i].value) {
                            p.push("@" + paras.keyword[i].name + index);
                            //参数化
                            _self.parameters.push({
                                "name": paras.keyword[i].name + index,
                                "sqlType": paras.keyword[i].sqlType || "VarChar",
                    ...
                    case '%':
                        ...
                        //左侧模糊匹配查询
                        _self.where += ("like @" + paras.keyword[i].name + index);
                        //参数化
                        _self.parameters.push({
                            "name": paras.keyword[i].name + index,
                            "sqlType": paras.keyword[i].sqlType || "VarChar",
                    ...
                    case '%%':
                        ...
                        break;
                    case 'is':
                        _self.where += ("is @" + paras.keyword[i].name + index);
                        //参数化
                        _self.parameters.push({
                            "name": paras.keyword[i].name + index,
                            "sqlType": paras.keyword[i].sqlType || "VarChar",
                ...
                //运算符
                _self.where += ( "=@" + paras.keyword[i].name + index);
                //参数化
                _self.parameters.push({
                    "name": paras.keyword[i].name + index,
                    "sqlType": paras.keyword[i].sqlType || "VarChar",
