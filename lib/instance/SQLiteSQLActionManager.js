...
// 语法处理
const GrammarMysql = require("../grammar/sqlite.js");
/**
 * MYSQL
    ...
    static queryEntityList(params, option = {"transaction": null}) {
        ...
        if (option && option.transaction) {
            return Promise.all([
                ...
                self.executeTransaction(count_sql, p.parameters, option.transaction,"all")
            ]).then(result => {
            ...
            return Promise.all([
                ...
                self.execute(count_sql, p.parameters,"all")
            ]).then(result => {
                return Promise.resolve({data: result[0], recordsTotal: result[1]});
            }).catch(ex => {
                console.log('执行execute查询数据列表出错', ex);
