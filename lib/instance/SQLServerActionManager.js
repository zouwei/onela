...
class MySQLActionManager extends BaseActionManager {
    ...
    static init(config) {
        ...
        // this.Request = require('tedious').Request;
        //     user: '...',
        //     password: '...',
        //     server: 'localhost',
        //     database: '...',
         ...
         * 创建连接池
         */
        this.conn = new sql.ConnectionPool(connectionCfg);
        // 错误跟踪
        sql.on('error', err => {
            // ... error handler
        ...
        return new Promise((resolve, reject) => {
            if (self.conn) {
                ...
                for (let i in parameters) {
                    request.input(parameters[i].name, parameters[i].sqlType, parameters[i].value)
                }
                // 执行
                request.query(sql)
                    .then(result => {
