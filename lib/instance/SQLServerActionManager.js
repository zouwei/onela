...
class SQLServerActionManager extends BaseActionManager {
    ...
    static createTransaction() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            conn.on('connect', function (err) {
                if (err) {
                    console.log("Error creating SQL Server transaction access connection", err); // 创建SQL Server事务访问连接出错
                } else {
                    console.log("SQL Server transaction access connection has been created"); // 已经创建SQL Server事务访问连接
    ...
    static execute(sql, parameters) {
        ...
        return new Promise((resolve, reject) => {
            ...
            conn.on('connect', function (err) {
                if (err) {
                    console.log("SQL Server access connection error", err); // SQL Server访问连接出错
                } else {
                    console.log("SQL Server access connection has been created"); // 已经创建SQL Server访问连接
                    // 每次单独创建连接
                    /**
                     * 执行SQL
                     */
                    ...
                    let request = new self.Request(sql, function (err, rowCount) {
                        //判断有没有出错
                        if (err) {
                            console.log("Error creating SQL execution request", err); // 创建SQL执行请求出错
                            return reject(err);
                        }
                    ...
                    request.on('error', function (err) {
                        console.log("SQL execution request error", error); // SQL执行请求出错
                    });
    ...
    static queryEntityList(params, option = { "transaction": null }) {
        ...
        if (option && option.transaction) {
            ...
            return self.executeTransaction(sql, p.parameters, option.transaction).then(data => {
                ...
                console.log('Error when executing execute query data list', ex); // 执行execute查询数据列表出错
    ...
    static insert(params, option = { "transaction": null }) {
        ...
        if (option && option.transaction) {
            ...
            // console.log("Start executing SQL write"); // 开始执行SQL写入
