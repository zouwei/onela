...
const getParameters = function (paras) {
    ...
    if (paras.limit && paras.limit.length > 0) {
        ...
        _self.parameters.push({ name: 'onelaPageIndex', sqlType: 'Int', value: offset }, { name: 'onelaPageSize', sqlType: 'Int', value: limit });
    }
    return _self;
};
/**
    ...
    for (const updateItem of paras.update || []) {
        ...
        if ('case_field' in updateItem && 'case_item' in updateItem) {
            ...
            for (const cw of case_item) {
                ...
                _self.parameters.push({ name: `${key}${index}_case`, sqlType: 'NVarChar', value: cw.case_value }, { name: `${key}${index}_val`, sqlType: sqlTypeHandle(updateItem.sqlType), value: cw.value });
                index++;
            }
            caseParts.push('END)');
